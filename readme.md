...
**git push --set-upstream origin HEAD**. Флаг **--set-upstream** — это «полная», или «длинная», версия флага **-u**. А **HEAD** здесь — это синоним текущей ветки.  
С тем же результатом можно было бы выполнить команду **git push -u origin feature-goose-poem**.
